...
namespace Bai09
{
    public partial class Form1 : Form
    {
        ...
        private bool isValidInfo()
        {
            ...
            if (studentID_Box.Text == "" || name_Box.Text == "" || majorComboBox.Text == "")
                isValid = false;
                ...
                foreach (DataGridViewRow row in infoList.Rows)
                    if (row.Cells[0].Value != null)
                        if (studentID_Box.Text == row.Cells[0].Value.ToString())
                        {
                            ...
                            row.Cells[2].Value = majorComboBox.SelectedItem.ToString();
                            row.Cells[3].Value = gender;
                            row.Cells[4].Value = rightBox.Items.Count.ToString();
                            MessageBox.Show("Cập nhật thông tin sinh viên thành công");
                            return;
                        }
                infoList.Rows.Add(studentID_Box.Text, name_Box.Text, majorComboBox.SelectedItem.ToString(), gender, rightBox.Items.Count.ToString());
            }
